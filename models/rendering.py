...
def bezier_render(all_points, all_widths, all_alphas, force_cpu=False,
    ...
    for k in range(bs):
        ...
        # white background
        image = raster[:, :, 3:4] * raster[:, :, :3] + th.ones(raster.shape[0], raster.shape[1], 3, device = pydiffvg.get_device()) * (1 - raster[:, :, 3:4])
        image = image[:, :, :3]
        image = image.permute(2, 0, 1).view(3, canvas_size, canvas_size)
